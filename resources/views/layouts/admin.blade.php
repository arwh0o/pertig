...
<head>
    ...
    <title>@yield('title', 'Admin Dashboard') - Pertigaan</title>
    ...
    <div class="flex h-screen overflow-hidden relative">
        ...
        <aside id="sidebar" class="w-64 bg-white border-r border-gray-200 flex-col fixed inset-y-0 left-0 z-50 transform -translate-x-full md:translate-x-0 md:static md:flex transition-transform duration-300 ease-in-out shadow-2xl md:shadow-none">
            ...
            <nav class="flex-1 overflow-y-auto p-4 space-y-2">
                <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('admin.dashboard') ? 'bg-primary/5 text-primary' : 'text-gray-600 hover:bg-gray-50 hover:text-primary' }} rounded-xl transition-all font-bold">
                    <span class="material-symbols-outlined">restaurant_menu</span>
                    Menu Manager
                </a>
                <a href="{{ route('gallery.index') }}" class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('gallery.index') ? 'bg-primary/5 text-primary' : 'text-gray-600 hover:bg-gray-50 hover:text-primary' }} rounded-xl transition-all font-bold">
                    <span class="material-symbols-outlined">photo_library</span>
                    Galeri Foto
                </a>
                <a href="{{ route('admin.feedbacks.index') }}" class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('admin.feedbacks.index') ? 'bg-primary/5 text-primary' : 'text-gray-600 hover:bg-gray-50 hover:text-primary' }} rounded-xl transition-all font-bold">
            ...
            <header class="bg-white border-b border-gray-200 sticky top-0 z-30">
                <div class="px-6 md:px-8 py-5 flex justify-between items-center gap-4">
                    <div class="flex items-center gap-3">
                        ...
                        <h2 class="text-xl md:text-2xl font-black truncate">@yield('header', 'Dashboard')</h2>
                    </div>
